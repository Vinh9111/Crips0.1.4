...
(______/|_____)  \__)____/|  __/ 
                          |_|    
      y/n thì bấm y enter
      n thì enter không cần ấn gì
""")
os.system("termux-setup-storage && apt update && apt upgrade && pkg install python && pip install requests && pip install bs4 && pip install colorama && pip install pystyle")
import sys
import time
import socket
import subprocess
...
║
║ ➢ Spam Sms
║ ➢ FACEBOOK: https://www.facebook.com/chongiukhann?mibextid=ZbWKwL
║ ➢ GROUP ZALO: https://zalo.me/g/dmqedp806    
║ ➣ Version: v0.1.4
║ ➣ Support: https://bio.link/huu_vinh
║ ➢ V-Tool 🔆
╚══════════════════════════════════════════════════════════════════════════╝
"""
...
class Tool:
    def __init__(self):
        self.log_file_path = "access_log.txt"
    ...
    def increment_access_count(self):
        with open(self.log_file_path, "r+") as log_file:
            access_count = int(log_file.read())
            access_count += 1
            log_file.seek(0)
            log_file.write(str(access_count))
    def get_access_count(self):
        with open(self.log_file_path, "r") as log_file:
            return int(log_file.read())
    def display_access_count(self):
        print(f" Lượt truy cập tool: {self.get_access_count()}")
def main():
    my_tool = Tool()
    def on_access():
        my_tool.increment_access_count()
        my_tool.display_access_count()
    on_access()
if __name__ == "__main__":
    main()
...
while not re.search("^(0?)(3[2-9]|5[6|8|9]|7[0|6-9]|8[0-6|8|9]|9[0-4|6-9])[0-9]{7}$",sdt):
  print(" \033[1;31mERROR:\033[1;32m Sai Định Dạng ! ")
  sdt = input(" [!] Nhập Lại Số Điện Thoại: ")
count = int(input(" [⟨⟩] ➩ Số Tin Nhắn / Call: "))
if sdt == "0932197401":
  print(" \033[1;31mERROR:\033[1;31m Số Admin ! ")
  exit()
...
def run(sdt,i):                  
  ...
  threading.submit(KILO,sdt)
  threading.submit(GAPO,sdt) 
  print("\033[91m[\033[1;32mÇrips\033[91m] \033[1;32mSend SMS - Call | Delay : 5 | Trạng Thái : Thành Công ",)  
  for j in range(0, 5):
    print(f"\033[91m[\033[1;32mÇrips\033[91m]\033[1;32mLOADING\r",end="")
